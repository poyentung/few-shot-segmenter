...
def set_greyscale_weights(model:vgg.VGG)->vgg.VGG:
    ...
    grayscale_weights = torch.zeros((kernel_out_channels, 1, kernel_rows, kernel_columns))
    grayscale_weights = (weights[:,0,:,:]* 0.2989) + (weights[:,1,:,:] * 0.5870) + (weights[:,2,:,:] * 0.1140)
...
class GlobalAveragePooling2D(nn.Module):
    ...
    def forward(self, support_annots, support_imgs_encoded): 
        ...
        support_annots_resized = transforms.Resize(encoded_input_size)(support_annots.squeeze(2)).unsqueeze(2) 
        
        # make the annot the size = [bs, nshot, c, w', h'] 
        support_annots_resized = torch.tile(support_annots_resized, (1,1,nchannels,1,1)) 
        support_imgs_masked = torch.multiply(support_imgs_encoded, support_annots_resized) # element-wise multiplication
        
        # AvgPool along [nshot,w',h'] -> size = [bs,1,c,1,1]
        support_imgs_masked = torch.div(support_imgs_masked.sum(dim=[1,3,4], keepdims=True),
        ...
        support_imgs_masked = torch.tile(support_imgs_masked.squeeze(1), (1,1,encoded_input_size[0], encoded_input_size[1])) # size = [bs,c,w',h']
