...
class HPNLearner(nn.Module):
    ...
    def interpolate_support_dims(self, hypercorr, spatial_size=None):
        bsz, ch, ha, wa, hb, wb = hypercorr.size()                                                     # [bs, 128, 16, 16, 2, 2]
        hypercorr = hypercorr.permute(0, 4, 5, 1, 2, 3).contiguous().view(bsz * hb * wb, ch, ha, wa)   # [bs, 2*2*128, 16, 16] 
        hypercorr = F.interpolate(hypercorr, spatial_size, mode='bilinear', align_corners=True)        #   -> [bs, 2*2*128, 32, 32]
        o_hb, o_wb = spatial_size
        hypercorr = hypercorr.view(bsz, hb, wb, ch, o_hb, o_wb).permute(0, 3, 4, 5, 1, 2).contiguous() # [bs, 128, 32, 32, 2, 2] 
        return hypercorr

    def forward(self, hypercorr_pyramid):

        # Encode hypercorrelations from each layer (Squeezing building blocks)
        hypercorr_sqz4 = self.encoder_layer4(hypercorr_pyramid[0]) # size = [bs, 128, 16, 16, 2, 2]
        hypercorr_sqz3 = self.encoder_layer3(hypercorr_pyramid[1]) # size = [bs, 128, 32, 32, 2, 2]
        hypercorr_sqz2 = self.encoder_layer2(hypercorr_pyramid[2]) # size = [bs, 128, 64, 64, 2, 2]

        # Propagate encoded 4D-tensor (Mixing building blocks)
        hypercorr_sqz4 = self.interpolate_support_dims(hypercorr_sqz4, hypercorr_sqz3.size()[-4:-2])  # [bs, 128, 16, 16, 2, 2] -> [bs, 128, 32, 32, 2, 2]
        hypercorr_mix43 = hypercorr_sqz4 + hypercorr_sqz3                                             # [bs, 128, 32, 32, 2, 2]
        hypercorr_mix43 = self.encoder_layer4to3(hypercorr_mix43)                                     # [bs, 128, 32, 32, 2, 2]

        hypercorr_mix43 = self.interpolate_support_dims(hypercorr_mix43, hypercorr_sqz2.size()[-4:-2]) # [bs, 128, 32, 32, 2, 2] -> [bs, 128, 64, 64, 2, 2]
        hypercorr_mix432 = hypercorr_mix43 + hypercorr_sqz2                                            # [bs, 128, 64, 64, 2, 2]
        hypercorr_mix432 = self.encoder_layer3to2(hypercorr_mix432)                                    # [bs, 128, 64, 64, 2, 2]

        bsz, ch, ha, wa, hb, wb = hypercorr_mix432.size()
        hypercorr_encoded = hypercorr_mix432.view(bsz, ch, ha, wa, -1).mean(dim=-1)                    # [bs, 128, 64, 64]
