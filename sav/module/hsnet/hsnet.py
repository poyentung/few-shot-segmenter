...
class HypercorrSqueezeNetwork(nn.Module):
    ...
    def forward(self, query_img, support_img, support_mask):
        with torch.no_grad():
            query_feats = self.extract_feats(query_img, self.backbone, self.feat_ids, self.bottleneck_ids, self.lids)     # [30 * [bs, c, ha, wa]]
            support_feats = self.extract_feats(support_img, self.backbone, self.feat_ids, self.bottleneck_ids, self.lids) # [30 * [bs, c, hb, wb]]
            support_feats = self.mask_feature(support_feats, support_mask.clone())                                        # [30 * [bs, c, hb, wb]]
            corr = Correlation.multilayer_correlation(query_feats, support_feats, self.stack_ids)                         # [3  * [bs, c, ha, wa, hb, wb]]
