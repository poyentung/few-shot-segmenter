...
class FewShotSegmenter(BaseModule):
    def __init__(
        ...
        if backbone == "vgg16":
            ...
            block3 = list(backbone.children())[0][
                10:16
            ]  # 256 filters (remove maxpool2d)
            block4 = list(backbone.children())[0][
                17:23
            ]  # 512 filters (remove maxpool2d)
            ...
            self.common_rep_conv = nn.Sequential(
                nn.Conv2d(256, 128, 3, 1, 1), nn.InstanceNorm2d(128), nn.ReLU()
            )
            ...
            self.backbone_encoder = nn.Sequential(*list(self.backbone.children())[:-1])[
                1
            ]
    ...
    def vit_forward(self, query_img, support_imgs):
        ...
        query_img_encoded = query_img.view(-1, 14, 14, 768).permute(
            0, 3, 1, 2
        )  # size = [bs,768,14,14]
        ...
        for support_img in support_imgs:
            ...
            # Expand the class token to the full batch
            batch_class_token = self.backbone.class_token.expand(
                support_img.size(0), -1, -1
            )
            support_img = torch.cat([batch_class_token, support_img], dim=1)
            support_img = self.backbone_encoder(support_img)[:, 1:]
            support_img_encoded = support_img.view(-1, 14, 14, 768).permute(
                0, 3, 1, 2
            )  # size = [nshot,768,14,14]
            support_imgs_encoded.append(support_img_encoded)

        return {
            "query_img_encoded": query_img_encoded,  # size = [bs,768,14,14]
            "support_imgs_encoded": torch.stack(support_imgs_encoded),
        }  # size = [bs,3,768,14,14]
